...
public class DataCurso {
	...
	public LinkedList<Curso> getCursosDisp(Alumno alu) {
		...
		try {
			stmt=DbConnector.getInstancia().getConn().prepareStatement(			
					  "select cm.idCurso,c.nombre nombre_curso,cm.idModalidad,m.nombre nombre_mod "
					+ "from curso_modalidad cm "
					+ "inner join curso c "
					+ "on c.id = cm.idCurso "
					+ "inner join modalidad m "
					+ "on m.id = cm.idModalidad "
					+ "where c.id >= ? "
					+ "order by c.id "
					+ "limit 2;"
					);
			...
			if(rs!=null) {
				while(rs.next()) {
					c = new Curso(rs.getInt("idCurso"), rs.getString("nombre_curso"),rs.getInt("idModalidad"), rs.getString("nombre_mod"));
					cursos.add(c);
				}
			}
		} catch (SQLException e) {
