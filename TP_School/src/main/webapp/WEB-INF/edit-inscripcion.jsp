...
<% Alumno a = (Alumno) session.getAttribute("user");
    Curso c = (Curso) request.getAttribute("disponibles");%>
</head>
...
%>
	<div class="bg-light"
		...
		<form action="EditarInscripcion" method="post">
			<div class="form-row">
				<div>
					<span style="color: #6c757d"><i
						class="fa fa-folder fa-2x pb-4 pt-3 pl-3"></i></span>
				</div>
				<div class="align-middle">
					<h1 class="text-secondary pb-4 pt-3 px-4" style="font-size: 1.5em">Editar
						Inscripcion</h1>
				</div>
			...
			<div class="form-row">
				<div class="col-md-2 mb-3">
					<label for="validationDefault01">Legajo</label> <input type="text"
						class="form-control" id="validationDefault01" name="legajo"
						required placeholder="Legajo"
						value="<%=session.getAttribute("user")!=null?a.getLegajo():""%>"
						<%=session.getAttribute("user")!=null?"disabled":"" %>>
				</div>
				<div class="col-md-4 mb-3 mx-2">
					<label for="cur">Curso</label> <select id="cur" name="cur" required
						class="form-select" onchange="cargaModalidad();"
						aria-label="Select course">
						<option value="" disabled>Seleccione curso...</option>
						<% if(c != null) { %>
						<option selected value="<%=c.getIdCurso()%>"><%=c.getNombre()%>
							año
						</option>
						<% } %>
					</select>
				...
				<div class="col-md-4 mb-3">
					<label for="mod">Modalidad</label> <select id="mod" name="mod"
						required class="form-select" onfocus="cargaModalidad();"
			...
			<div class="row align-bottom">
				<button class="btn btn-lg btn-primary btn-block mt-4 ml-3 mr-4"
					...
					<span><i class="fa fa-pen"></i></span> Editar
				</button>
	...
	<script>
			function cargaModalidad() {
                ...
                $.getJSON("CursoServlet", //.. este es el servlet
